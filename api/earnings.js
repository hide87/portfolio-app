...
const earningsHistory = s?.earningsHistory?.history?.map(h => ({ date: h.quarter?.fmt, epsActual: h.epsActual?.raw, epsEstimate: h.epsEstimate?.raw, surprisePct: h.surprisePercent?.raw })) || []
const earningsDate = s?.calendarEvents?.earnings?.earningsDate?.[0]?.fmt || null
const quarterlyEPS = s?.earnings?.financialsChart?.quarterly?.map(q => ({ date: q.date, actual: q.actual?.raw, estimate: q.estimate?.raw })) || []
return new Response(JSON.stringify({ earningsHistory, earningsDate, quarterlyEPS }), { headers: { ‘Content-Type’: ‘application/json’, ‘Access-Control-Allow-Origin’: ’*’ } })
} catch (e) {
